...
export function TtsControls({ status, supported, hasText, onPlay, onPause, onResume, onStop }: TtsControlsProps) {
  ...
  // rather than a dead button, just don't offer it there. Piper voices
  // don't need this check (WASM, not SpeechSynthesis), but "system" is the
  // default voice, so this guard still applies most of the time.
  if (!supported) return null;

  return (
    <div className="flex items-center gap-1">
      {status === "loading" ? (
        // The Piper voice model downloads (~60MB, once per voice, cached
        // after) and loads before anything can play -- a spinner here beats
        // a play button that looks broken for however long that takes.
